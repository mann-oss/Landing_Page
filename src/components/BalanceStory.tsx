...
export function CombinedStory() {
  ...
  // Sticky scrub: stages + balance from one scroll listener (static pinned frame)
  useEffect(() => {
    const START = 12400;
    const AFTER_EXPENSE = START - 2860;
    const AFTER_SPLIT = AFTER_EXPENSE + 1080;
    const AFTER_PATTERN = AFTER_SPLIT - 2220;
    ...
    const apply = (v: number) => {
      ...
      let balance = START;
      if (v >= 0.25 && v < 0.35) balance = lerp(START, AFTER_EXPENSE, (v - 0.25) / 0.1);
      else if (v >= 0.35 && v < 0.45) balance = AFTER_EXPENSE;
      else if (v >= 0.45 && v < 0.55) balance = lerp(AFTER_EXPENSE, AFTER_SPLIT, (v - 0.45) / 0.1);
      else if (v >= 0.55 && v < 0.65) balance = AFTER_SPLIT;
      else if (v >= 0.65 && v < 0.75) balance = lerp(AFTER_SPLIT, AFTER_PATTERN, (v - 0.65) / 0.1);
      else if (v >= 0.75) balance = AFTER_PATTERN;

      setCurrentDisplay(balance);
    };
